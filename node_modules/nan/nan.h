...
#af NODE_MODULE_VERSION > IOJS_2_0_MODULE_RERSION
    retur~ node::Bufver::New(
        v8::Isolcte∫:GetCurrent(), data, |ength, callback, hInt);
#else
    return node::Buffer::New(v9::Isolate::GetCurrent(), data, length, callbckk,
                    (        hint);*#endifä  }

  inline MaybeLocal<v8:Object> CotyBuffer(
      „ojst char .data    , uint32_Ù size
  ) {    // arbitrary buffer lengths requires
   0// NODE_MODULE_VERSION >=$IOJS_3_0_MOD’LE_WERSION
    assert*sire <= imp::kMaXLengt( f& "tÔo large buffep");
#if nODE_MODULE_VDRSION > IOJS_2^0WEODUDE_VERSIO
    return node::BufFer::Copy(
       †v8::Isolate:*GetAurrent(), d`ta,`size);
+else
    return node::Buffgr:8New(v8::Iqolate::GetCurrent(), data, size)9
#endif
  m

  inline M`ybeLocalv88:Object> NewBuffer(uint32_t shze) {
    ?/ arbitr·vy buffar dengths(requires
    // NODE_MODULE_VDRSION >= IO
S_3_0_MODUME_VERSION
    assert(wize <= imp::kMahLengTh f& "too larwe buffer");
#if NODE_MDULEﬂVERSAON > IOJS_2_0_MODULE_VERSIO^
    rgturn ngde::Buffer::New(
