...
class Sprinkler {
  ...
  runPrograms() { 
    ...
    setTimeout(this.runPrograms.bind(this), ((Math.floor(new Date().getTime() / 1000) + 1) * 1000) - new Date().getTime())
  ...
  turnOff(station) {
    if (this.stations[station].status === 1 && this.stations[station].duration <= 0) {
      console.log(`${this.stations[station].name} turned off at ${new Date().toLocaleString()}`)
  ...
  turnOn(station) {
    if (this.stations[station].status === 0) {
      console.log(`${this.stations[station].name} turned on at ${new Date().toLocaleString()}`)
      this.stations[station].status = 1;
      rpio.write(this.stations[station].pin, rpio.HIGH);
    }
  };

  updatePrograms(data) { //this is a promise to handle changing the program from the clients browser in index.js
    return new Promise(function(resolve, reject) {
      fs.writeFile('./programs.json', JSON.stringify(data, null, 1), 'utf-8', function(err) {
        if (err) {
